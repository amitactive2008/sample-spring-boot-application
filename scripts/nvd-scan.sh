...
Modes:
  ...
  scan                 Update once, then scan all Java services (default)
...
Options:
  ...
  --skip-update        Scan from an initialized cache without accessing NVD
...
update_database() {
  ...
  if [[ -n "$nvd_api_key" ]]; then
    ...
    echo "Warning: NVD_API_KEY is unset; updates may be heavily rate-limited." >&2
  fi
...
if [[ "$skip_update" != "true" ]]; then
  update_database
elif [[ ! -f "${data_directory}/odc.mv.db" ]]; then
